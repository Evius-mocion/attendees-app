...
export const EventCheckin = (props: Props) => {
    ...
    return (
		<Stack w={'100%'} justify='center' align='center' h={'100%'}>
			{action === '' && <AccessByAction setAction={setAction} />}
			{action === 'email' && <AccessByEmail actionError={actionError} setAction={setAction} />}
			{action === 'qr' && <AccessByQR actionError={actionError} setAction={setAction} />}
			{action === 'register' && (
				<Stack gap={'xl'} w={'80%'}>
					{additionalFields ? <RegisterFormByEvent /> : <RegisterFormByEventAdditionalFields />}
				</Stack>
			)}
			{action !== '' && (
				<Group justify='end' w={'80%'}>
					...
					{additionalFields && action === 'register' ? (
						<Button size='lg' onClick={() => setAdditionalFields(false)} /* disabled={actionError} */>
							Siguiente
						</Button>
					) : (
						<Button
